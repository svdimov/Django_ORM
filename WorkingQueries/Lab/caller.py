...
def filter_authors_by_nationalities(nationalities) -> str:
    authors = Author.objects.filter(nationality=nationalities).order_by('first_name', 'last_name')
...
def filter_authors_by_birth_year(start, end) -> str:
    author = Author.objects.filter(birth_date__year__range=[start,end]).order_by('-birth_date')
